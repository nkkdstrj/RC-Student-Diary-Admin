...
function Editsubbtn   ({
  ...
  const [subButtonNameError, setSubButtonNameError] = useState('');
 ...
 setErrorname('');
  ...
  useEffect(() => {
 const pdfLink = initialPdflink;
 const decodedUrl = decodeURIComponent(pdfLink);
 const parts = decodedUrl.split('/');
 const fileName = parts[parts.length - 1];
 const title = fileName.split('.')[0];
 const titleWithExtension = title + '.pdf';

 const audio = initialAudio;
 const decodededUrl = decodeURIComponent(audio);
 const partss = decodededUrl.split('/');
 const audioeName = partss[partss.length - 1];
 const audiotitle = audioeName.split('.')[0];
 const audiotitleWithExtension = audiotitle + '';
 setAudioName(audiotitleWithExtension || '')
    setTextFromPdf(initialContent || '');
    setDownloadUrl(initialPdflink || '');
    setAudioDownloadUrl(initialAudio || '');
    
    setSubButtonName(  initialSubButtonName  || '');
    setFileName( titleWithExtension || '');

    if (initialPdflink) {
      setShowDisplaySection(true);
    }
     
  }, [initialContent, initialPdflink, initialAudio, initialLayout, initialSubButtonName]);


  const clearAudio = () => {
    setAudioDownloadUrl("");
    setAudioName(""); // Set the audio field to an empty string
  };
const stylebtn={
  ...
  const handleFileChange = (e) => {
    ...
    if (file) {
      ...
      // Handle the file as needed (e.g., upload to server, process, etc.)
    }
  ...
  const handleUpload = () => {
    ...
    // Execute both upload tasks concurrently
  ...
  const handleSave = () => {
    ...
    if (downloadUrl && audioDownloadUrl) {
      layout = 1;
    } else if (downloadUrl && !audioDownloadUrl && downloadable) {
      layout = 2;
    } else if (downloadUrl && !audioDownloadUrl && !downloadable) {
      layout = 3;
    }
    ...
    onSave({
      ...
      audio: audioLink,
      pdflink: downloadUrl,
      subuttonkeyy: initialsubButtonKey,
      buttonkeyy: initialbuttonKey,
    });
  ...
  const handleCancel = () => {
    ...
    if (downloadUrl) {
      const storageRef = ref(storage, downloadUrl); // Assuming downloadUrl contains the file path
      deleteObject(storageRef)
        .then(() => {
          // Deletion successful, reset relevant states
          setDownloadUrl('');
          setUploadProgress(0);
    ...
    // Delete uploaded audio file if it exists
    if (audioDownloadUrl) {
      const audioRef = ref(storage, "Audio/" + selectedAudio.name);
      deleteFile(audioRef);
    }

    // Close the FileUpload component
    setShow(false); // Define this function to close the FileUpload component
  };
  ...
  const handleCheckboxChange = (e) => {
    ...
    // Hide audio file input when checkbox is checked
    if (e.target.checked) {
      setSelectedAudio(null);
    }
  };
  return (
    <>
      ...
      <button onClick={() => {
    handleShow();
  }}  className="btn text-secondary"  style={{fontSize: "13px",padding:'0px', marginLeft: '5px' }}>
          <BsFillPencilFill/>
      </button>
          ...
          <Form.Group>
            ...
            <Form.Control
              ...
              onChange={(e) => {
                ...
                setSubButtonNameError(''); // Clear validation message on input change
              }}
...
>
  ...
  <div style={{ display: 'flex', justifyContent: 'center', alignItems: 'center', marginLeft:'1rem'}}>
     <button onClick={clearAudio } className="btn text-secondary" style={{ fontSize: "15px", padding: '0px', marginLeft: '5px' }}>
        <span><AiFillCloseCircle /></span>
      </button>
  <span
            style={{ cursor: "pointer", color: "#1e329c" ,marginLeft:'1rem'}}
